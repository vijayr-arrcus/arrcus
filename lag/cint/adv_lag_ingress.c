...
tx 1 psrc=16 data=0x00000000000f000000000007080045000046000100004011f66021212103212121010d050cfb003264476161616161616161616161616161616161616161616161616161616161616161616161616161616161
...
int trunk_create(bcm_trunk_t tid, int member_port_num, int *member_ports)
{
  ...
  if (rv != BCM_E_NONE) {
      printf("\n Control port set failed for trunk %d", rv);
  }
  ...
  for (i=0; i < member_port_num; i++) {
      ...
      if (rv != BCM_E_NONE) {
          printf("\n Control port set failed for port %d(i=%d) %d", member_ports[i],i, rv);
      }
  }
  /* Prepare local ports to gports */
  for (i=0; i<member_port_num; i++)
  ...
  */
  ...
  rv = bcm_port_get(unit, tgport, &flags, &pi, &pm);
  if (rv != BCM_E_NONE) {
      printf("\n port get for trunk failed %d", rv);
  }
...
main_trunk( int port_0, int port_1,  bcm_trunk_t tid, int vlan_domain)
{
  ...
  if (rv != BCM_E_NONE) {
      printf("\n port class set failed for trunk %d", rv);
  }
		   ...
		   int tagged, int lif_id )
{
  ...
  if (!tagged) {
    ...
    // This sets untagged packets default vlan for the incoming port
      ...
      if (rv != BCM_E_NONE) {
          printf("\n printf untagged vlan set failed %d",rv);
      }
    // Init AC and Drop AC
    //
  }
  ...
  rv = bcm_vlan_port_create(unit, &vp);
  if (rv != BCM_E_NONE) {
      printf("\n printf vlan port create failed %d",rv);
  }
  ...
  if (rv != BCM_E_NONE) {
      printf("\n vlan port translation failed %d",rv);
  }
  ...
  if (rv != BCM_E_NONE) {
      printf("\n vlan port translation failed-2 %d",rv);
  }
...
add_trunk_to_vlan( int tid, int vlan_id, int vsi_id, int tagged, int lif_id )
{
  ...
  BCM_GPORT_TRUNK_SET(tgport, tid);
  set_tpid( tgport );
  rv = add_gport_to_vlan( tgport, vlan_id, vsi_id, tagged, lif_id );
  if (rv != BCM_E_NONE) {
      printf("\n add gport to vlan for trunk failed %d",rv);
  }
  return 0;
}
...
create_vlan_vsi( int vlan_id, int vsi_id )
{
  ...
  if (rv != BCM_E_NONE) {
      printf("\n vswitch create failed - %d", vsi_id);
  }
  //create VLAN
  rv = bcm_vlan_create(unit, vlan_id);
  if (rv != BCM_E_NONE) {
      printf("\n vlan create failed - %d", vsi_id);
  }
  return 0;
}
