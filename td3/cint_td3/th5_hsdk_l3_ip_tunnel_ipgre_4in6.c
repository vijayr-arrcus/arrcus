 ...
 *
 * Usage    : BCM.0> cint th5_hsdk_l3_ip_tunnel_ipgre_4in6.c
 *
 * Config   : bcm78900_b0-generic-l3.yml
 *
 * Log file : th5_hsdk_l3_ip_tunnel_ipgre_4in6_log.txt
 *
 * Test Topology :
 *                      +---------------------+
 *                      |                     |
...
int testCleanup(int unit)
{
    ...
    rv = bcm_l3_intf_stat_detach(unit, egr_l3_if_netwrk.l3a_intf_id);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_l3_intf_stat_detach %s\n", bcm_errmsg(rv));
        return rv;
    }
    rv = bcm_tunnel_terminator_stat_detach(unit, tnl_term_info);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_tunnel_terminator_stat_detach %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_flexctr_action_destroy(unit, stat_counter_id_initiator);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_flexctr_action_destroy %s\n", bcm_errmsg(rv));
        return rv;
    }
    rv = bcm_flexctr_action_destroy(unit, stat_counter_id_terminator);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_flexctr_action_destroy %s\n", bcm_errmsg(rv));
        return rv;
    }
