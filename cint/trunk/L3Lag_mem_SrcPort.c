...
{
    bcm_gport_t sysport=0,modport=0, modport_1=0, modport_2=0, modport_gport;
    int                 rv= BCM_E_NONE;
    int                 i;
...
//
//    rv = bcm_port_get(unit, )

   bcm_trunk_pp_port_allocation_info_t_init(&trunk_pp_port_aloc_info_get);
   bcm_trunk_pp_port_allocation_get(unit, tid, 0, &trunk_pp_port_aloc_info_get);
   ...
   printf("Allocated pp_port2 is %d. \n", trunk_pp_port_aloc_info_get.pp_port_per_core_array[1]);
    ...
    int core = 0;
    int modid = my_modid+core;
    int pp_port = trunk_pp_port_aloc_info_get.pp_port_per_core_array[0];
...
/*
    ...
    BCM_GPORT_MODPORT_SET(modport_gport, modid, pp_port);
    bcm_stk_sysport_gport_set(unit, sysport, modport_gport);

*/
    int sysport_arr[2] = {0x1235, 0x1236};

    for (i = 0; i < out_local_port_num; i++) {
        BCM_GPORT_SYSTEM_PORT_ID_SET(tmp_gport, out_local_ports[i]);
        rv = bcm_stk_sysport_gport_set(unit, tmp_gport, -1);

        BCM_GPORT_SYSTEM_PORT_ID_SET(sysport, sysport_arr[i]);
        BCM_GPORT_MODPORT_SET(modport_gport, modid, out_local_ports[i]);
        bcm_stk_sysport_gport_set(unit, sysport, modport_gport);

        BCM_GPORT_SYSTEM_PORT_ID_SET(tmp_gport,out_local_ports[i] );
        bcm_stk_sysport_gport_set(unit, tmp_gport, modport_gport);
    }
    printf("Have set sysport for front panel ports \n");
