...
int create_subinterface( bcm_gport_t gport, int vlan_id, int vsi_id, int lif_id, int my_mac, int mtu)
{
...
int create_subinterface_and_nh( bcm_gport_t gport, int vlan_id, int vsi_id, int lif_id, int my_mac, int nhmac )
{
  ...
  int l3if  = create_subinterface(gport, vlan_id, vsi_id, lif_id, my_mac, 1524);
...
int create_mpls_tnl_swap(int unit, bcm_if_t oif, bcm_mpls_label_t egress_lbl, bcm_if_t *tnl_if)
{
  bcm_mpls_egress_label_t egress_lbl_info;

  bcm_mpls_egress_label_t_init(&egress_lbl_info);
  egress_lbl_info.flags = BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT;
  egress_lbl_info.egress_qos_model.egress_ttl = bcmQosEgressModelPipeMyNameSpace;
  egress_lbl_info.label = egress_lbl;
  egress_lbl_info.l3_intf_id = oif;
  bcm_mpls_tunnel_initiator_create(unit, 0, 1/* num lbls */, &egress_lbl_info);
  print("Create tunnel init %d", egress_lbl_info.tunnel_id);
  *tnl_if = egress_lbl_info.tunnel_id;
  return 0;
}
...
// trunk_fec_id_subintf1 = create_trunk_subinterface_and_nh( 100, 50, 1050, 500, 2, 5 );
int trunk_mpls_isr (int tid, int vlan_id, int my_mac,
                    int nh_mac, int lif_id, int vsi_id,
                    int in_label, int out_label)
{
  ...
  BCM_TRUNK_ID_SET(tid, 0, tid);
  BCM_GPORT_TRUNK_SET(tgport, tid);
  int l3if = create_subinterface(tgport, vlan_id, vsi_id, lif_id, my_mac, mtu);
  // Create MPLS tunnel encap
// L3if below is used as the OutRIF and the tnl_if is out_l2_lif
//  create_mpls_tnl_swap(unit, 0/*RIF will be programmed in arp - egress_only*/, 
//                       out_label/* lbl */, &tnl_if);
  create_mpls_tnl_swap(unit, l3if/*RIF will be programmed in arp - egress_only*/, 
                       out_label/* lbl */, &tnl_if);
  create_egr_nh(unit, tnl_if, l3if, nh_mac, &fec);
  create_ing_nh(unit, tnl_if, tgport, &fec);

  create_ilm_swap(unit, in_label. fec);
}
